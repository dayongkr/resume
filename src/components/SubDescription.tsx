...
export function SubDescription({
  date,
  link,
}: Readonly<{ date?: { from: Date; to?: Date | 'current' }; link?: Link }>) {
  return (
    <p className="not-prose text-sm opacity-80 md:text-base">
      {date && `${formatDateString(date.from)}`}
      {date && date.to && ` ~ ${formatDateString(date.to)}`}
      {link && (
        <>
          {' / '}
          <LinkBlank href={link.href}>{link.title}</LinkBlank>
        </>
      )}
    </p>
  );
